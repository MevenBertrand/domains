...
(**)
  ...
  Section termmodel.
    ...
    Proof.
      symmetry. unfold subst_denote at 2. unfold varmap_denote.
      apply finprod_universal. intros.
      rewrite (cat_assoc PLT).
      ...
      unfold subst_denote, varmap_denote.
      rewrite finprod_proj_commute.
      auto.
    Qed.
    ...
    Proof.
      symmetry. unfold subst_denote, varmap_denote.
      apply finprod_universal.
      intros.
    ...
    Proof.
      ...
      intros.
      rewrite (cat_assoc PLT).
      unfold subst_denote, varmap_denote at 1.
    ...
    Proof.
      ...
      generalize (tm_denote_var ((x',σ)::Γ') i0).
      simpl.
      simpl in *.
      set (p' := proj ((x',σ)::Γ') i0). 
...
cut (forall HASDF,
   (forall a0 H, HASDF a0 H ≈ castty (weaken_fresh Γ Γ' σ x' Hx i0 a0 H) ∘ p) ->
   match
     lookup i0 Γ' as a0
     return (lookup i0 Γ' = a0 -> PLT.hom (cxt ((x', σ) :: Γ')) (ty a0))
   with
   | Some a0 =>
       fun H0 : lookup i0 Γ' = Some a0 =>  HASDF a0 H0 
   | None =>
       fun _ : lookup i0 Γ' = None =>
       PLT.terminate false (cxt ((x', σ) :: Γ'))
   end Logic.eq_refl
   ≈ match
       string_dec x' i0 as Hi
       return
         (PLT.hom (ty (if Hi then Some σ else lookup i0 Γ'))
            (ty (lookup i0 Γ')))
     with
     | left H0 =>
         castty
           (eq_ind i0
              (fun y : string => lookup y Γ' = None -> None = lookup i0 Γ')
              (fun H1 : lookup i0 Γ' = None => eq_sym H1) x' 
              (eq_sym H0) Hx') ∘
     ...
     intros. 
     apply H1.
     clear H1. intros. apply H0.
     clear H0.

     revert p.
     unfold inenv. simpl.
     unfold weaken_fresh. simpl.
     unfold inenv. simpl.
     destruct (string_dec x' i0).
     subst i0.
     simpl; intros.
     clear H0.
     generalize (Logic.eq_refl (lookup x' Γ')).
...
Proof.
  symmetry.
  unfold weaken_denote, varmap_denote at 1.
  ...
  cut
( forall HASDF HASDF2,
  (forall a H0, HASDF a H0  ≈ castty (env_incl_wk Γ Γ' x σ H i a H0) ∘ proj ((x,σ)::Γ') i) ->
  (forall i0 a H0, HASDF2 i0 a H0 ≈ castty (H i0 a H0) ∘ proj Γ' i0) ->

   match
     (if string_dec x i then Some σ else lookup i Γ) as a
     return
       ((if string_dec x i then Some σ else lookup i Γ) = a ->
        PLT.hom (cxt ((x, σ) :: Γ')) (ty a))
   with
   | Some a =>
       fun H0 : (if string_dec x i then Some σ else lookup i Γ) = Some a =>
         HASDF a H0
   | None =>
       fun _ : (if string_dec x i then Some σ else lookup i Γ) = None =>
       PLT.terminate false (cxt ((x, σ) :: Γ'))
   end Logic.eq_refl ∘
    mk_finprod ((x, σ) :: Γ') (cxt Γ' × F σ)
      (fun i' : string =>
       if string_dec x i' as Hi
        return
          (PLT.hom (cxt Γ' × F σ) (ty (if Hi then Some σ else lookup i' Γ')))
       then π₂
       else proj Γ' i' ∘ π₁)
   ≈ (if string_dec x i as Hi
       return
      ...
      PLT.pair_map
        (mk_finprod Γ (cxt Γ')
           (fun i0 : string =>
            match
              lookup i0 Γ as a
              return (lookup i0 Γ = a -> PLT.hom (cxt Γ') (ty a))
            with
            | Some a =>
                fun H0 : lookup i0 Γ = Some a => HASDF2 i0 a H0
(*                tm_denote Γ' a (tm_var Γ' i0 a (H i0 a H0))*)
            | None =>
                fun _ : lookup i0 Γ = None => PLT.terminate false (cxt Γ')
            end Logic.eq_refl)) id
).
  intros. apply H0.
  clear H0. intros. apply tm_denote_var. 
  clear H0. intros. apply tm_denote_var. 

  intros.
  generalize (finprod_proj_commute ((x,σ)::Γ') i (cxt Γ' × F σ)
    (fun i' : string =>
       if string_dec x i' as Hi
        return
          (PLT.hom (cxt Γ' × F σ) (ty (if Hi then Some σ else lookup i' Γ')))
  ...
  generalize (finprod_proj_commute Γ i (cxt Γ')
    (fun i1 : string =>
          match
            lookup i1 Γ as a
            return (lookup i1 Γ = a -> PLT.hom (cxt Γ') (ty a))
          with
          | Some a => fun H4 : lookup i1 Γ = Some a => HASDF2 i1 a H4
          | None =>
              fun _ : lookup i1 Γ = None => PLT.terminate false (cxt Γ')
          end Logic.eq_refl)).

  revert HASDF HASDF2 H0 H1.
  generalize (env_incl_wk Γ Γ' x σ H i).
  generalize (proj ((x,σ)::Γ') i).
  generalize (proj Γ).
  unfold inenv. simpl.
  pattern (string_dec x i).
  destruct (string_dec x i).
  ...
  apply cat_respects; auto.
  refine (cast_dec_id false _ ty _ _ (i0 σ Logic.eq_refl)).
  ...
  rewrite (cat_assoc PLT).
  rewrite H2.
  generalize (Logic.eq_refl (lookup i Γ)).
  
  pattern (lookup i Γ) at 2 3 4 5 9 10.
  case (lookup i Γ); intros.
  rewrite H0.
  rewrite <- (cat_assoc PLT).
  rewrite H3.
  ...
  rewrite <- (cat_assoc PLT).
  apply cat_respects; auto.
  generalize (i0 a e) (H i a e). unfold inenv.
  rewrite (i0 a e). intros.
  etransitivity.
  refine (cast_dec_id false _ ty _ _ e0).
  decide equality. apply Adec.
  symmetry.
...
Proof.
  ...
  rewrite <- (cat_assoc PLT).
  rewrite finprod_proj_commute.
  match goal with [ |- _ ∘ (_ ?X) ≈ _ ] => generalize X end.
  generalize (H x σ i).
  revert i. unfold inenv.
  pattern (lookup x Γ) at 1 3 4 5 6.
  ...
  apply Eqdep_dec.UIP_dec. decide equality. apply Adec.
  apply Eqdep_dec.UIP_dec. decide equality. apply Adec.
  apply Eqdep_dec.UIP_dec. decide equality. apply Adec.
  discriminate.
Qed.
...
Proof.
  ...
  rewrite (varmap_extend_bind Γ ((fresh[Γ'],σ)::Γ')
    (weaken_map tm tm_wk Γ Γ' VAR (fresh_atom (‖Γ'‖ ++ nil)%list) σ
...
Proof.
  ...
  generalize i.
  generalize (VAR0 x).
  rewrite i. simpl. intros.
  rewrite tm_denote_var.
  rewrite tm_denote_var.
  rewrite (cat_assoc PLT).
  apply cat_respects; auto.
  etransitivity.
  apply (cast_compose false (option A) ty _ _ _ _ i1).
  match goal with [ |- castty ?X ≈ castty ?Y ] => 
    generalize X Y
  end.
  unfold inenv. rewrite (i0 σ i1).
  intros. replace e with i2. auto.
...
Lemma term_subst_soundness Γ τ m : forall Γ' (VAR:varmap tm Γ Γ'),
  ...
  tm_denote _ _ m ∘ subst_denote Γ Γ' VAR.
Proof.
  intros. apply tm_traverse_correct.
  intros. simpl.
  symmetry. apply varmap_shift_bind.
  intros. symmetry. apply weaken_term_denote.
  ...
  generalize (VAR0 x).
  rewrite i. simpl. intros.
  replace i0 with (Logic.eq_refl (Some σ)).
  rewrite (cast_refl false (option A) ty).
  rewrite (cat_ident2 PLT). auto.
  apply Eqdep_dec.UIP_dec. decide equality. apply Adec.
Qed.  
...
Lemma subst_soundness Γ x σ₁ σ₂ n₁ n₂ :
  ...
  tm_denote _ _ (subst Γ σ₂ σ₁ x n₁ n₂).
Proof.
  apply subst_soundness_lemma. intros. apply term_subst_soundness.
Qed.  
  End termmodel.
