...
Section powerdom.
    ...
    Proof.
      ...
      - unfold INV.
        ...
        + destruct IHc as [z0 [?[?[?[?]]]]].
          ...
          destruct (H0 a) as [xq [??]]; [ apply cons_elem; auto; fail |].
          destruct (H2 a) as [yq [??]]; [ apply cons_elem; auto; fail |].
    ...
    Next Obligation.
      ...
      - intros.      
        ...
        + destruct (fmap_convex_swell z x y) as [z0 [?[?[?[?[??]]]]]]; auto.
          ...
          split. { split; hnf; simpl; intros; auto. }
          split. { split; hnf; simpl; intros; auto. }
          apply erel_image_elem.
          rewrite (fmap_rel_elem Convex).
  ...
  Proof.
    unfold single_rel. rewrite esubset_dec_elem.
    - simpl.
  ...
  Proof.
    unfold union_rel. rewrite esubset_dec_elem.
    - simpl. intuition.
  ...
  Proof.
    ...
    - apply PLT.compose_hom_rel in H.    
      ...
      exists (single_elem X x).
      split.
      { simpl. apply (single_rel_elem sort X). auto. }
      simpl.
      rewrite (fmap_rel_elem X Y f sort).
      destruct sort; simpl.
            ...
            { clear H.
              ...
              - destruct IHc as [zs [?[??]]].
                ...
                + destruct (Ha a) as [z [?[??]]]; clear Ha. apply cons_elem; auto.
                  ...
                  * apply cons_elem in H5. destruct H5.
                    econstructor.
                    ** split. apply cons_elem; left. reflexivity.
                       simpl. split; intros.
                       destruct H5 as [[??][??]].
...
Proof.
  ...
  - destruct a as [x y].
    ...
    + apply PLT.pair_hom_rel.
      split; auto.
      simpl. rewrite (empty_rel_elem sort X Y). auto.
...
Proof.
  ...
  - apply PLT.compose_hom_rel in H.
    ...
    { assert (forall ys:finset Y,
      ...
      { induction ys.
        - intros. clear H2. destruct H1 as [_ H1]. hnf in H1. simpl in H1.
