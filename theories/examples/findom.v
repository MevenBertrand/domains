...
(* Note: cannot use equations as it doesn't support mutual definitions *)
...
(* Compatible elements have the same universe level???
   No, this is not true b/c bot is compatible with any term.
 *)
...
Definition valid_append f g : 
  ...
  -> valid_fun (f ++ g).
Proof.
  move=> /andP [/andP [/andP [Cf Nbf] Nf] Vf] /andP [/andP [/andP [Cg Nbg] Ng] Vg] Cfg.
  ...
  - unfold compatible_fun in *.
    ...
    + eapply (Cgi _ Igj).
  - have L: forall f g, no_bot_result f ->
                   no_bot_result g ->
                   no_bot_result (f ++ g).
    { clear. intros f g Nf Ng.
...
(* This proof probably needs to follow the definition of le. *)
Lemma le_refl : forall u, le u u.
eapply elt_rect' with 
  (P := fun u => le u u)
  (Pf := fun l => le_fun l l).
all: try solve [intros;reflexivity].
- intros. cbn. eapply Nat.eqb_refl.
- intros e h. rewrite le_succ; eauto.
- intros e he l hl. rewrite le_tpi; rewrite he; eauto.
- intros l hl. rewrite le_abs. auto.
- intros u v l hu hv hl. cbn.
  rewrite hu.
Admitted.

Lemma le_trans : forall u v, le u v -> forall w, le v w -> le u w.
Admitted.
...
Proof.
  ...
  - move=> /andP [h1 /andP [ /andP [h2 h3] h4]].
