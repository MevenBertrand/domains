...
Fixpoint proj_tpi' (proj : elt -> elt -> elt) 
  (a: elt) (b:elt)
  (f : list (elt * elt))  : list (elt * elt) := 
    match f with 
    | nil => nil 
    | cons (u,v) ps => 
        cons (proj u a, proj v b) (proj_tpi' proj a b ps)
    end.

Fixpoint proj_fun' (proj : elt -> elt -> elt)
        (a : elt) (f : list (elt * elt)) 
        (g : list (elt * elt))  : list (elt * elt) := 
    match g with 
    | nil => nil 
    | cons (u,v) ps => 
        let x := proj u a  in
...
Definition wf_complexity := 
  wf_lexprod nat (fun _ => nat) Nat.lt (fun x => Nat.lt) lt_wf (fun _ => lt_wf).
*)
...
Proof.
  ...
  have ih_proj_tpi: forall f,
    ...
    k < m -> forall a b, (rk_fun f) <= k -> 
            proj_tpi' (proj' k) a b f = 
            proj_tpi' (proj' (rk_fun f)) a b f.
  { induction f as [|[u v]f].
    - intros. done.
    - intros k Lt a b. cbn.
      move=> Le.
...
Definition proj u a : elt := proj' (rk u) u a.

Lemma proj_bot_ty u : proj u bot = u.
destruct u; try done.
Qed.
